...
export async function getAllPortfolio() {
  return client.fetch(
    `*[_type == "portfolio"] | order(overview.completedDate desc) {
      _id,
      title,
      slug,
      client,
...
export async function getFeaturedPortfolio() {
  return client.fetch(
    `*[_type == "portfolio" && featured == true] | order(completedAt desc)[0...6] {
      _id,
      title,
      slug,
      client,
      ...
      "projectUrl": overview.projectUrl,
      featured
    }`
  )
}
